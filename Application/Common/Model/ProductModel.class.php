...
class ProductModel extends Model {
    ...
    public function find($id) {
     ...
     public function findById($id) {
        $data = $this->_db->where('id='.$id)->field('id,name,price,img')->find();
        return $data;
    }   
    ...
    public function getProductsByClassid($class_id){
        $data=array(
            'class_id'=>$class_id,
            'status'=>1,
        );
        return $this->_db->where($data)->order('sell desc')->limit(0,5)->select();
    }

    //减少库存
    public function upProCount($id,$count){
        if(!$id || !is_numeric($id)) {
            throw_exception("ID 不合法");
        }
        if(!is_numeric($count)) {
        ...
        $this->_db->where($data)->setDec('count',$count);
        $this->_db->where($data)->setInc('sell',$count);
    }

    public function getById($id){
        $data=array(
                'class_id'=>array('eq',$id),
                'status'=>array('eq',1),
            );
        return $this->_db->where($data)->select();
    } 
