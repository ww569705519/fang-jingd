		...
		<div class="form-group">
			...
			<div class="col-md-4">
				<select class="form-control" name="class_id">
					...
					<?php if(is_array($class)): foreach($class as $key=>$v): ?><option value="<?php echo ($v["class_id"]); ?>" <?php if($products["class_id"] == $v['class_id']): ?>selected<?php endif; ?>><?php echo ($v["classname"]); ?></option><?php endforeach; endif; ?>
				</select>
			</div>
		</div>
		<div class="form-group">
			...
			<div class="col-sm-5">
				...
				<img style="display: none" id="upload_org_code_img" src="<?php echo ($products["img"]); ?>" width="150" height="150">
				<input id="file_upload_image" name="img" type="hidden" multiple="true" value="<?php echo ($products["img"]); ?>"></div>
		</div>
		<div class="form-group">
			<label class="col-md-2 control-label">商品详情</label>
			<div class="col-md-4">
				<textarea class="input js-editor" id="editor_singcms" name="info" rows="20"><?php echo ($products["info"]); ?></textarea>
			</div>
		</div>
		<div class="form-group">
			<label class="col-md-2 control-label">商品库存</label>
			<div class="col-md-2">
				<input type="text" class="form-control"  placeholder="请输入商品库存" name="count" value="<?php echo ($products["count"]); ?>"></div>
		</div>
		<div class="form-group">
			<label class="col-md-2 control-label">是否上架</label>
			<div class="col-md-4">
				<label class="radio-inline">
					<input type="radio" name="shelf" id="inlineRadio1" value="1" <?php if($products["shelf"] == 1): ?>checked<?php endif; ?>>上架</label>
				<label class="radio-inline">
					<input type="radio" name="shelf" id="inlineRadio2" value="0" <?php if($products["shelf"] == 0): ?>checked<?php endif; ?>>下架</label>
			</div>
		</div>
