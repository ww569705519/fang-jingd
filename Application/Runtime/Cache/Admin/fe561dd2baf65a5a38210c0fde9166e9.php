...
<div id="page-wrapper" >
	...
	<div class="padding10">
						...
						<?php if(is_array($class)): $i = 0; $__LIST__ = $class;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><tr>
								...
								<td>
									<span  attr-status="<?php if($v['status'] == 1): ?>0
										<?php else: ?>
										1<?php endif; ?>
									"  attr-id="<?php echo ($v["class_id"]); ?>" class="sing_cursor singcms-on-off" id="singcms-on-off" ><?php echo (status($v["status"])); ?>
								</span>
							</td>
							<td>
								<a href="<?php echo U('Classify/edit',array('class_id'=> $v['class_id']));?>" >
									<span class="glyphicon glyphicon-record" ></span>
								</a>
								<input type="hidden" name="class_id" value="<?php echo ($v["class"]["id"]); ?>">
								<a href="javascript:void(0)" attr-id="<?php echo ($v["class_id"]); ?>" id="singcms-delete"  attr-a="admin" attr-message="删除">
	    ...
	    var SCOPE = {
	        'save_url' : '/index.php/admin/Classify/add',
